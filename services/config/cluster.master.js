module.exports = (cluster, isDebug) => {
    ...
    const createFolder = folderPath => !fs.existsSync(folderPath) && fs.mkdirSync(folderPath);
    createFolder(path.join(__dirname, '../asset'));
    createFolder(path.join(__dirname, '../asset/bundle'));

    const workers = {},
        numWorkers = 1; //isDebug ? 1 : require('os').cpus().length;
