// Table name: QT_CONG_TAC_TRONG_NUOC { id, shcc }
const keys = ['ID'];
const obj2Db = { 'id': 'ID', 'shcc': 'SHCC' };
...
module.exports = app => {
    app.model.qtCongTacTrongNuoc = {
        ...
        get: (condition, selectedColumns, orderBy, done) => {
            ...
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT * FROM QT_CONG_TAC_TRONG_NUOC' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ') WHERE ROWNUM=1';
        ...
        getAll: (condition, selectedColumns, orderBy, done) => {
            ...
            condition = app.dbConnection.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM QT_CONG_TAC_TRONG_NUOC' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '');
        ...
        getPage: (pageNumber, pageSize, condition, selectedColumns, orderBy, done) => {
            ...
            app.dbConnection.execute(sql_count, parameter, (err, res) => {
                ...
                const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT QT_CONG_TAC_TRONG_NUOC.*, ROW_NUMBER() OVER (ORDER BY ' + (orderBy ? orderBy : keys) + ') R FROM QT_CONG_TAC_TRONG_NUOC' + (condition.statement ? ' WHERE ' + condition.statement : '') + ') WHERE R BETWEEN ' + (leftIndex + 1) + ' and ' + (leftIndex + pageSize);
                app.dbConnection.execute(sql, parameter, (error, resultSet) => {
