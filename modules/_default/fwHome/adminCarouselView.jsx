...
class CarouselPage extends React.Component {
    ...
    delete = (e, item) => {
        T.confirm('Xóa tập hình ảnh', 'Bạn có chắc bạn muốn xóa tập hình ảnh này?', true, isConfirm => isConfirm && this.props.deleteCarousel(item.id, item.priority));
        e.preventDefault();
    }
    ...
    render() {
        ...
        if (this.props.carousel && this.props.carousel.page && this.props.carousel.page.list && this.props.carousel.page.list.length > 0) {
                    ...
                    <tbody>
                        {this.props.carousel.page.list.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    <Link to={'/user/carousel/edit/' + item.id} data-id={item.id}>{item.title}</Link>
                                </td>
                                <td style={{ textAlign: 'right' }}>{item.height}</td>
                                <td className='toggle' style={{ textAlign: 'center' }}>
                                    <label>
                                        <input type='checkbox' checked={item.single} onChange={() => permissionWrite && this.props.updateCarousel(item.id, { single: item.single == 1 ? 0 : 1 })} />
                                        <span className='button-indecator' />
                                    </label>
                                </td>
                                <td className='toggle' style={{ textAlign: 'center' }}>
                                    <label>
                                        <input type='checkbox' checked={item.active} onChange={() => permissionWrite && this.props.updateCarousel(item.id, { active: item.active == 1 ? 0 : 1 })} />
                                        <span className='button-indecator' />
                                    </label>
                                </td>
                                <td>
                                    <div className='btn-group'>
                                        <Link to={'/user/carousel/edit/' + item.id} data-id={item.id} className='btn btn-primary'>
            ...
            table = <p key={0}>Không có tập ảnh!</p>;
        }

        return [
            table,
            <button key={1} type='button' className='btn btn-primary btn-circle' style={{ position: 'fixed', right: '10px', bottom: '10px' }}
                onClick={this.create}>
