...
class VideoModal extends React.Component {
    ...
    show = (video) => {
        let { id, title, link, image, content } = video ? video : { id: null, title: '', link: '', image: '', content: '' };
        ...
        $('#videoLink').val(link);
        this.imageBox.current.setData('video:' + (id ? id : 'new'));
    ...
    save = (event) => {
        ...
        if (changes.title.vi == '') {
            ...
            if (id) {
                this.props.updateVideo(id, changes, () => {
...
class VideoPage extends React.Component {
    ...
    edit = (e, item) => {
        this.props.getVideo(item.id, video => this.videoModal.current.show(video));
        e.preventDefault();
    }

    changeActive = (item) => {
        this.props.updateVideo(item.id, { active: !item.active });
    }

    delete = (e, item) => {
        T.confirm('Video: Xóa video', 'Bạn có chắc bạn muốn xóa video này?', true, isConfirm => isConfirm && this.props.deleteVideo(item.id));
        e.preventDefault();
    }
    ...
    render() {
        const currentPermissions = this.props.system && this.props.system.user && this.props.system.user.permissions ? this.props.system.user.permissions : [],
            ...
            permissionDelete = currentPermissions.includes('component:delete');
        ...
        const components = [
            ...
            <VideoModal key={2} createVideo={this.props.createVideo} updateVideo={this.props.updateVideo} ref={this.videoModal} readOnly={!permissionWrite} />
        ];
        if (permissionWrite) {
            components.push(
