module.exports = app => {
    app.post('/api/staff/qt-ky-luat', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyLuat.create(req.body.data, (error, item) => res.send({ error, item })));

    app.put('/api/staff/qt-ky-luat', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyLuat.update({ id: req.body.id }, req.body.changes, (error, item) => res.send({ error, item })));

    app.delete('/api/staff/qt-ky-luat', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyLuat.delete({ id: req.body.id }, (error) => res.send(error)));

    app.post('/api/user/staff/qt-ky-luat', app.permission.check('staff:login'), (req, res) => {
    ...
    app.put('/api/user/staff/qt-ky-luat', app.permission.check('staff:login'), (req, res) => {
    ...
    app.delete('/api/user/staff/qt-ky-luat', app.permission.check('staff:login'), (req, res) => {
        if (req.session.user) {
            app.model.qtKyLuat.get({ id: req.body.id }, (error, item) => {
                if (error || item == null) {
                    res.status(400).send({ error: 'Not found!' });
