    ...
    app.post('/api/student/admin/bhyt', app.permission.orCheck('developer:login'), async (req, res) => {
        try {
            ...
            if (currentBhyt) {
                    ...
                    if (!item) res.send({ error: 'Lỗi hệ thống' });
                        ...
                        if (data.dienDong != null && item.dienDong != data.dienDong) {
                            ...
                            if (mapperDienDong[item.dienDong])
                                loaiPhi = app.model.tcHocPhiDetail.get({ namHoc, hocKy, mssv, loaiPhi: mapperDienDong[item.dienDong] });
                            ...
                            await app.model.tcHocPhi.update({ namHoc, hocKy, mssv }, {
                                ...
                                congNo: parseInt(congNo) + parseInt(congNo) + diff,
                                ngayTao: thoiGian
                            });
                            if (loaiPhi)
                ...
                throw 'Sinh viên chưa tham gia bảo hiêm y tế';
        } catch (error) {
            res.send({ error });
        }
    });
