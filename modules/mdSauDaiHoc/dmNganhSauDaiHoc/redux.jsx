...
export function getDmNganhSdhPage(pageNumber, pageSize, pageCondition, done) {
    ...
    return dispatch => {
        const url = `/api/sau-dai-hoc/danh-sach-nganh/page/${page.pageNumber}/${page.pageSize}`;
...
export function getDmNganhSdhAll(done) {
    return dispatch => {
        const url = '/api/sau-dai-hoc/danh-sach-nganh/all';
        T.get(url, data => {
            if (data.error) {
                T.notify('Lấy danh sách ngành sau đại học bị lỗi' + (data.error.message && (':<br>' + data.error.message)), 'danger');
                console.error(`GET: ${url}.`, data.error);
...
export function getDmNganhSdh(ma, done) {
    return () => {
        const url = `/api/sau-dai-hoc/danh-sach-nganh/item/${ma}`;
        T.get(url, { ma }, data => {
            if (data.error) {
...
export function createDmNganhSdh(item, done) {
    return dispatch => {
        const url = '/api/sau-dai-hoc/danh-sach-nganh';
...
export function createDmNganhSdhMutiple(data, done) {
    return (dispatch) => {
        const url = '/api/sau-dai-hoc/danh-sach-nganh/multiple';
