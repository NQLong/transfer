// Table name: FW_SINH_VIEN_SDH { ma, ho, ten, gioiTinh, ngaySinh, danToc, tonGiao, quocTich, hienTaiSoNha, noiSinhMaTinh, maKhoa, maNganh, namTuyenSinh, nienKhoa, bacDaoTao, chuongTrinhDaoTao, sdtCaNhan, sdtLienHe, email, coQuan, gvhd, tenDeTai, tinhTrang, heDaoTao }
const keys = ['MA'];
const obj2Db = { 'ma': 'MA', 'ho': 'HO', 'ten': 'TEN', 'gioiTinh': 'GIOI_TINH', 'ngaySinh': 'NGAY_SINH', 'danToc': 'DAN_TOC', 'tonGiao': 'TON_GIAO', 'quocTich': 'QUOC_TICH', 'hienTaiSoNha': 'HIEN_TAI_SO_NHA', 'noiSinhMaTinh': 'NOI_SINH_MA_TINH', 'maKhoa': 'MA_KHOA', 'maNganh': 'MA_NGANH', 'namTuyenSinh': 'NAM_TUYEN_SINH', 'nienKhoa': 'NIEN_KHOA', 'bacDaoTao': 'BAC_DAO_TAO', 'chuongTrinhDaoTao': 'CHUONG_TRINH_DAO_TAO', 'sdtCaNhan': 'SDT_CA_NHAN', 'sdtLienHe': 'SDT_LIEN_HE', 'email': 'EMAIL', 'coQuan': 'CO_QUAN', 'gvhd': 'GVHD', 'tenDeTai': 'TEN_DE_TAI', 'tinhTrang': 'TINH_TRANG', 'heDaoTao': 'HE_DAO_TAO' };
...
module.exports = app => {
    app.model.fwSinhVienSdh = {
        ...
        searchPage: (pagenumber, pagesize, listfaculty, listnganh, listfromcity, listethnic, listnationality, listreligion, listtinhtrangsinhvien, gender, searchterm, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN :ret:=fw_sv_sdh_search_page(:pagenumber, :pagesize, :listfaculty, :listnganh, :listfromcity, :listethnic, :listnationality, :listreligion, :listtinhtrangsinhvien, :gender, :searchterm, :totalitem, :pagetotal); END;',
                { ret: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR }, pagenumber: { val: pagenumber, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, pagesize: { val: pagesize, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, listfaculty, listnganh, listfromcity, listethnic, listnationality, listreligion, listtinhtrangsinhvien, gender, searchterm, totalitem: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER }, pagetotal: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER } }, (error, result) => app.database.oracle.fetchRowsFromCursor(error, result, (error, result) => {
                    if (error) {
                        done && done(error);
                        reject(error);
                    } else {
