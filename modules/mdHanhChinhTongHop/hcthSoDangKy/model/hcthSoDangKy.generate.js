...
module.exports = app => {
    app.model.hcthSoDangKy = {
        ...
        createSoVanBan: (donvigui, capvanban, loaivanban, nam, tudong, ngaytao, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN hcth_so_dang_ky_create_so_van_ban(:donvigui, :capvanban, :loaivanban, :nam, :tudong, :ngaytao); END;',
                { donvigui, capvanban, loaivanban, nam, tudong, ngaytao }, (error, result) => {
                    if (error) {
                        done && done(error);
                        reject(error);
                    } else {
        ...
        validateSoCongVan: (sodangky, capvanban, donvigui, nam, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN hcth_so_dang_ky_validate_so_cong_van(:sodangky, :capvanban, :donvigui, :nam); END;',
                { sodangky, capvanban, donvigui, nam }, (error, result) => {
                    if (error) {
                        done && done(error);
                        reject(error);
                    } else {
        ...
        searchPage: (pagenumber, pagesize, tabvalue, searchterm, done) => new Promise((resolve, reject) => {
            app.database.oracle.connection.main.executeExtra('BEGIN :ret:=hcth_so_dang_ky_search_page(:pagenumber, :pagesize, :tabvalue, :searchterm, :totalitem, :pagetotal); END;',
                { ret: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR }, pagenumber: { val: pagenumber, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, pagesize: { val: pagesize, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, tabvalue, searchterm, totalitem: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER }, pagetotal: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER } }, (error, result) => app.database.oracle.fetchRowsFromCursor(error, result, (error, result) => {
                    if (error) {
                        done && done(error);
                        reject(error);
                    } else {
