...
export class CanBoNhan extends AdminPage {
    ...
    getAdapter = () => {
        ...
        if (currentPermissions.some(item => ['hcth:manage', 'rectors:login'].includes(item)) || this.state.lienPhong)
            ...
            return SelectAdapter_FwCanBoByDonVi(listDonVi.map(item => item.maDonVi) || this.props.system?.user?.staff.maDonVi);
        }
        return SelectAdapter_FwCanBo;
    }
    ...
    onDelete = (item) => {
        T.confirm('Xóa cán bộ tham gia', `Bạn có chắc bạn muốn xóa cán bộ ${(item.hoCanBoNhan + ' ' + item.tenCanBoNhan).trim().normalizedName()}?`, true,
            isConfirm => isConfirm && this.props.removeCanBoNhanNhiemVu(item.id, () => {
                ...
                this.props.getList({ ma: this.props.nhiemVuId, ids: currentId });
            })
        );
    }

    tableCanBoNhan = () => renderTable({
        getDataSource: () => this.props.hcthNhiemVu?.item?.canBoNhan || (!this.props.nhiemVuId && []),
        stickyHead: false,
        ...
        renderRow: (item, index) => {
            ...
            return (
                <tr key={index}>
                    ...
                    <TableCell type='text' style={{ whiteSpace: 'nowrap' }} content={(item.hoCanBoNhan + ' ' + item.tenCanBoNhan).trim().normalizedName()} />
    ...
    onSubmit = () => {
        ...
        if (shccs.length == 0) {
            ...
            this.props.create(this.props.nhiemVuId, shccs, vaiTro, (items) => {
                this.canBoNhan.clear();
                this.props.getList({ ma: this.props.nhiemVuId, ids: [...currentId, ...items.map(item => item.id)] });
            });
        }
    }

    getRoleData = () => {
        return this.props.isCreator ? vaiTroSelector : [{ id: vaiTro.PARTICIPANT.id, text: vaiTro.PARTICIPANT.text }];
    }

    render() {
        const userPermission = this.props.system?.user?.permissions || []
        const { permissions = {}, isCreator, isManager, data, isNew = true } = this.props || {};
        const userShcc = this.props.system?.user?.shcc;
        const dsDonViQuanLi = (this.props.system?.user?.staff?.donViQuanLi || []).map(item => item.maDonVi);
        return (
            <div className='tile'>
                <div className='form-group'>
                    ...
                    <div className='tile-body row'>
                        {this.canAdd() && (<div className='col-md-12 row'>
                            <FormSelect className='col-md-6' multiple={true} ref={e => this.canBoNhan = e} data={SelectAdapter_FwCanBoByDonVi(dsDonViQuanLi)} placeholder="Chọn cán bộ tham gia" />
...
class CongVanDiSelector extends React.Component {
    ...
    changeSearch = () => {
        const
            ...
            searchTerm = this.search.value(),
            year = this.year.value(),
            [fromTime, toTime] = Number.isInteger(Number(year)) ? getYearRange(Number(year)) : [],
    ...
    render() {
        ...
        return (<div className='col-md-12' >
            <div className='form-group row'>
                <FormTextBox label='Tìm kiếm' ref={e => this.search = e} className='col-md-6' />
                <FormTextBox label='Trạng thái' ref={e => this.status = e} className='col-md-3' />
                <FormTextBox label='year ' ref={e => this.year = e} className='col-md-3' />
                <div className='col-md-12' style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', gap: '10px' }}>
                    <button type='submit' className='btn btn-danger' onClick={this.resetSearch}>
                        <i className='fa fa-lg fa-times-circle-o' />Hủy tìm kiếm
                    </button>
...
export class LienKet extends React.Component {
        ...
        return renderTable({
            ...
            renderRow: (item, index) => {
                if (item.loaiB == loaiLienKet.CONG_VAN_DEN.id)
                    return this.renderCongVanDenRow(item, index, sitePermission.deleteLienKet);
                else if (item.loaiB == loaiLienKet.CONG_VAN_DI.id)
                    return this.renderCongVanDiRow(item, index, sitePermission.deleteLienKet);
            }
        });
    }
    ...
    render() {
        return (<div className='tile'>
            ...
            <LienKetModal {...this.props} ref={e => this.modal = e} permission={this.props.permission} create={this.props.createLienKet} update={this.props.updateLienKet} get={this.props.getLienKet} permissions={this.props.currentPermissions} nhiemVuId={this.props.target} />
        </div>)
    }
}