// Table name: HCTH_NHIEM_VU { id, nguoiTao, donViNhan, noiDung, tieuDe, ngayBatDau, ngayKetThuc, doUuTien, ngayTao, lienPhong, trangThai, tienDo }
const keys = ['ID'];
...
module.exports = app => {
    app.model.hcthNhiemVu = {
            ...
            if (statement.length == 0) {
                ...
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.hcthNhiemVu.get({ rowId: resultSet.lastRowid }, done);
                    } else {
        ...
        getPage: (pageNumber, pageSize, condition, selectedColumns, orderBy, done) => {
            ...
            const sql_count = 'SELECT COUNT(*) FROM HCTH_NHIEM_VU' + (condition.statement ? ' WHERE ' + condition.statement : '');
