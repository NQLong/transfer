module.exports = (app) => {
  ...
  const staffMenu = {
    ...
    menus: {
      502: {
        ...
        link: '/user/hcth/cong-van-cac-phong',
  ...
  app.get(
    '/user/cong-van-cac-phong',
    app.permission.check('staff:login'),
    app.templates.admin
  );
  app.get(
    '/user/cong-van-cac-phong/:id',
    app.permission.check('staff:login'),
    app.templates.admin
  );
  app.get(
    '/user/hcth/cong-van-cac-phong',
    app.permission.check('hcthCongVanDi:read'),
    app.templates.admin
  );
  app.get(
    '/user/hcth/cong-van-cac-phong/:id',
    app.permission.check('hcthCongVanDi:read'),
    app.templates.admin
  );
  ...
  // APIs ----------------------------------------------------------------------------------------------------------------------------------------
  app.get(
    '/api/hcth/cong-van-cac-phong/search/page/:pageNumber/:pageSize',
    app.permission.check('staff:login'),
    (req, res) => {
      ...
      donViXem =
        donViXem.map((item) => item.maDonVi).toString() ||
        (permissions.includes('donViCongVanDi:manage') &&
          req.session?.user?.staff?.maDonVi) ||
        '';
      canBoXem = req.session?.user?.shcc || '';

      let loaiCanBo = rectorsPermission.login
        ? 1
        : hcthPermission.login
        ? 2
        : 0;

      if (
        rectorsPermission.login ||
        hcthPermission.login ||
        (!user.isStaff && !user.isStudent)
      ) {
        donViXem = '';
        canBoXem = '';
      }

      app.model.hcthCongVanDi.searchPage(
        pageNumber,
        pageSize,
        canBoNhan,
        donViGui,
  ...
  const getCurrentPermissions = (req) =>
    (req.session && req.session.user && req.session.user.permissions) || [];
  ...
  const getUserPermission = (
    ...
    const permission = {},
      currentPermissions = getCurrentPermissions(req);
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/all',
    app.permission.check('hcthCongVanDi:read'),
    (req, res) => {
      app.model.hcthCongVanDi.getAll((error, items) =>
  ...
  const createListDonViNhan = (listDonViNhan, congVanId, done) => {
    if (listDonViNhan && listDonViNhan.length > 0) {
      const promises = listDonViNhan.map(
        (donViNhan) =>
          new Promise((resolve, reject) =>
            app.model.hcthDonViNhan.create(
              {
          ...
          updateListFile(fileList, id, ({ error }) => {
            if (error) {
              ...
              if (donViNhanNgoai && donViNhanNgoai.length > 0) {
                listDonViNhanNgoai = donViNhanNgoai.map((id) => ({
                  id: id,
                  donViNhanNgoai: 1,
                }));
              }
              if (donViNhan && donViNhan.length > 0) {
                listDonViNhan = donViNhan.map((id) => ({
                  id: id,
                  donViNhanNgoai: 0,
                }));
              }
              ...
              createListDonViNhan(
                [...listDonViNhan, ...listDonViNhanNgoai],
                id,
                ({ error }) => {
                  if (error) {
                    throw error;
  ...
  const updateListFile = (listFile, congVanId, done) => {
    if (listFile && listFile.length > 0) {
      const [{ id, ...changes }] = listFile.splice(0, 1),
        sourcePath = app.path.join(
          app.assetPath,
          `/congVanDi/new/${changes.ten}`
        ),
        destPath = app.path.join(
          app.assetPath,
          `/congVanDi/${congVanId}/${changes.ten}`
        );
      if (!changes.ma)
        app.fs.rename(sourcePath, destPath, (error) => {
          if (error) done && done({ error });
          else {
            app.model.hcthFile.update(
              { id },
              { ...changes, ma: congVanId },
              (error) => {
                if (error) done && done({ error });
                else {
  ...
  // Cần sửa lại
  app.put(
    '/api/hcth/cong-van-cac-phong',
    app.permission.check('staff:login'),
    (req, res) => {
      const { fileList, donViNhan, donViNhanNgoai, ...changes } =
        req.body.changes;
      ...
      if (isSend) {
        ...
        app.model.hcthCongVanDi.updateSoCongVanDi(
          ma,
          donViGui,
          nam,
          (errors, result) => {
            if (errors) {
              ...
              app.model.hcthCongVanDi.update(
                { id: req.body.id },
                { trangThai },
                (errors, item) => {
                  if (errors) res.send({ errors, item });
                  else {
                    let listDonViNhan = [];
                    let listDonViNhanNgoai = [];
                    if (donViNhanNgoai && donViNhanNgoai.length > 0) {
                    ...
                    app.model.hcthDonViNhan.delete(
                      { ma: req.body.id, loai: CONG_VAN_DI_TYPE },
                      () =>
                        createListDonViNhan(
                          [...listDonViNhan, ...listDonViNhanNgoai],
                          req.body.id,
                      ...
                      createListDonViNhan(
                        ...
                        () => {
                          updateListFile(fileList, req.body.id, () => {
                            ...
                            let hanhDong;
                            if (
                              trangThaiBefore == trangThaiCongVanDi.MOI.id &&
                              [
                                trangThaiCongVanDi.CHO_KIEM_TRA.id,
                                trangThaiCongVanDi.DA_GUI.id,
                              ].includes(trangThaiAfter)
                            ) {
  ...
  app.delete(
    '/api/hcth/cong-van-cac-phong',
    app.permission.check('hcthCongVanDi:delete'),
    (req, res) => {
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/page/:pageNumber/:pageSize',
    app.permission.check('staff:login'),
    (req, res) => {
      const pageNumber = parseInt(req.params.pageNumber),
        pageSize = parseInt(req.params.pageSize);
      ...
      app.model.hcthCongVanDi.getPage(
        pageNumber,
        pageSize,
        condition,
        (error, page) => {
  ...
  app.uploadHooks.add('hcthCongVanDiFile', (req, fields, files, params, done) =>
    app.permission.has(
      req,
      () => hcthCongVanDiFile(req, fields, files, params, done),
      done,
      'staff:login'
    )
  );
  ...
  const hcthCongVanDiFile = (req, fields, files, params, done) => {
    if (
      ...
      const srcPath = files.hcthCongVanDiFile[0].path,
  ...
  //Delete file
  app.put(
    '/api/hcth/cong-van-cac-phong/delete-file',
    app.permission.check('hcthCongVanDi:delete'),
    (req, res) => {
      const id = req.body.id,
        ...
        congVan = id || null,
        filePath =
          app.assetPath + '/congVanDi/' + (id ? id + '/' : 'new/') + file;
      app.model.hcthFile.delete({ id: fileId, ma: congVan }, (error) => {
        if (error) {
          res.send({ error });
        } else {
          if (app.fs.existsSync(filePath)) app.deleteFile(filePath);
          res.send({ error: null });
        }
      });
    }
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/download/:id/:fileName',
    app.permission.check('hcthCongVanDi:read'),
    async (req, res) => {
      try {
        const { id, fileName } = req.params;
        const congVan = await app.model.hcthCongVanDi.getCVD({ id });
        const donViNhan = await app.model.hcthDonViNhan.getAllDVN(
          { ma: id, loai: 'DI' },
          'donViNhan',
          'id'
        );
        if (!(await isRelated(congVan, donViNhan, req))) {
          ...
          if (app.fs.existsSync(dir)) {
            const serverFileNames = app.fs
              .readdirSync(dir)
              .filter((v) => app.fs.lstatSync(app.path.join(dir, v)).isFile());
            for (const serverFileName of serverFileNames) {
              const clientFileIndex = serverFileName.indexOf(fileName);
              if (
                clientFileIndex !== -1 &&
                serverFileName.slice(clientFileIndex) === fileName
              ) {
        ...
        res
          ...
          .send(error.message || 'Không tìm thấy tập tin');
      }
    }
  );
  ...
  const isRelated = async (congVan, donViNhan, req) => {
    try {
      ...
      if (
        ...
        permissions.includes('hcth:login')
      ) {
        return true;
      }
      if (req.query.nhiemVu) {
        ...
        if (
          (await app.hcthNhiemVu.checkNhiemVuPermission(
            req,
            null,
            req.query.nhiemVu
          )) &&
      ...
      if (canBoNhan && canBoNhan.split(',').includes(req.session.user.shcc))
        ...
        donViGui == maDonVi &&
        (permissions.includes('donViCongVanDi:manage') ||
          maDonViQuanLy.find((item) => donViGui.includes(item.maDonVi)))
      ) {
        return true;
      } else {
        let maDonViNhan = donViNhan.map((item) => item.donViNhan);
        return (
          maDonViQuanLy.find((item) => maDonViNhan.includes(item.maDonVi)) ||
          (permissions.includes('donViCongVanDi:manage') &&
            maDonViNhan.includes(Number(req.session.user.staff?.maDonVi)))
        );
      }
    } catch {
      return false;
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/:id',
    app.permission.check('staff:login'),
    async (req, res) => {
      try {
        ...
        const donViNhan = await app.model.hcthDonViNhan.getAllDVN(
          { ma: id, loai: 'DI' },
          'id, donViNhan, donViNhanNgoai',
          'id'
        );
        if (!(await isRelated(congVan, donViNhan, req))) {
          throw { status: 401, message: 'permission denied' };
        }
        const files = await app.model.hcthFile.getAllFile(
          { ma: id, loai: 'DI' },
          '*',
          'thoiGian'
        );
        ...
        res.send({
          item: {
            ...
            donViNhan: (donViNhan
              ? donViNhan
                  .filter((item) => item.donViNhanNgoai == 0)
                  .map((item) => item.donViNhan)
              : []
            ).toString(),
            donViNhanNgoai: (donViNhan
              ? donViNhan
                  .filter((item) => item.donViNhanNgoai == 1)
                  .map((item) => item.donViNhan)
              : []
            ).toString(),
            listFile: files || [],
  ...
  app.post(
    '/api/hcth/cong-van-cac-phong/phan-hoi',
    app.permission.check('staff:login'),
    (req, res) => {
      ...
      app.model.hcthPhanHoi.create(newPhanHoi, (error, item) =>
        res.send({ error, item })
      );
    }
  );
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/lich-su/:id',
    app.permission.check('staff:login'),
    (req, res) => {
      app.model.hcthHistory.getAllFrom(
        parseInt(req.params.id),
        'DI',
        (error, item) => res.send({ error, item: item?.rows || [] })
      );
    }
  );
  ...
  const createStaffNotification = (item, status) =>
    new Promise((resolve, reject) => {
        ...
        if (error) return reject(error);
          ...
          createNotification(
            emails,
            {
              ...
              link: `/user/cong-van-cac-phong/${item.id}`,
            },
            (error) => {
  ...
  const createHcthStaffNotification = (item, status) =>
    new Promise((resolve, reject) => {
        ...
        if (error) reject(error);
          ...
          createNotification(
            emails,
            {
              title: 'Công văn đi',
              icon: 'fa-book',
              subTitle: 'Bạn có một công văn đi cần kiểm tra',
              iconColor: getIconColor(status),
              link: `/user/hcth/cong-van-cac-phong/${item.id}`,
            },
            (error) => {
              if (error) reject(error);
              else resolve();
  ...
  // Đang gửi cho cô Lan
  const createSchoolAdministratorNotification = (item, status) =>
    new Promise((resolve, reject) => {
      app.model.canBo.get({ shcc: '001.0068' }, 'email', '', (error, staff) => {
        if (error) reject(error);
          ...
          app.notification.send({
            ...
            title: 'Công văn đi',
            icon: 'fa-book',
            subTitle: 'Bạn có một công văn cần duyệt',
            iconColor: getIconColor(status),
            link: `/user/cong-van-cac-phong/${item.id}`,
          });
          resolve();
  ...
  // Đang gửi trả lại
  const createAuthorNotification = (id, shcc, status) =>
    new Promise((resolve, reject) => {
      app.model.canBo.get({ shcc: shcc }, 'email', '', (error, staff) => {
        if (error) reject(error);
          ...
          app.notification.send({
            ...
            title: 'Công văn đi',
            icon: 'fa-book',
            subTitle: 'Bạn có một công văn bị trả lại',
            iconColor: getIconColor(status),
            link: `/user/cong-van-cac-phong/${id}`,
          });
          resolve();
        }
      });
  ...
  app.assignRoleHooks.addRoles(quanLyCongVanDiRole, {
    id: 'donViCongVanDi:manage',
    text: 'Quản lý công văn các phòng trong đơn vị',
  });
  ...
  app.assignRoleHooks.addHook(quanLyCongVanDiRole, async (req, roles) => {
    ...
    if (
      req.query.nhomRole &&
      req.query.nhomRole === quanLyCongVanDiRole &&
      userPermissions.includes('manager:write')
    ) {
      const assignRolesList = app.assignRoleHooks
        .get(quanLyCongVanDiRole)
        .map((item) => item.id);
  ...
  app.permissionHooks.add(
    'assignRole',
    'checkRoleQuanLyCongVanDiTrongDonVi',
    (user, assignRoles) =>
      new Promise((resolve) => {
        const inScopeRoles = assignRoles.filter(
          (role) => role.nhomRole === quanLyCongVanDiRole
        );
  ...
  app.assignRoleHooks.addRoles(hcthQuanLyCongVanDiRole, {
    id: 'hcthCongVanDi:manage',
    text: 'Hành chính - Tổng hợp: Quản lý Công văn các phòng',
  });
  ...
  app.assignRoleHooks.addHook(hcthQuanLyCongVanDiRole, async (req, roles) => {
    ...
    if (
      req.query.nhomRole &&
      req.query.nhomRole == hcthQuanLyCongVanDiRole &&
      userPermissions.includes('manager:write')
    ) {
      const assignRolesList = app.assignRoleHooks
        .get(hcthQuanLyCongVanDiRole)
        .map((item) => item.id);
      return roles && roles.length && assignRolesList.contains(roles);
    }
  });
  ...
  app.permissionHooks.add(
    'assignRole',
    'checkRoleHcthQuanLyCongVanDi',
    (user, assignRoles) =>
      new Promise((resolve) => {
        const inScopeRoles = assignRoles.filter(
          (role) => role.nhomRole == hcthQuanLyCongVanDiRole
        );
        inScopeRoles.forEach((role) => {
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/selector/page/:pageNumber/:pageSize',
    app.permission.check('staff:login'),
    (req, res) => {
      const pageNumber = parseInt(req.params.pageNumber);
      ...
      const donViCanBo = (req.session?.user?.staff?.donViQuanly || []).map(
        (item) => item.maDonVi
      );
      const userPermissions = req.session.user?.permissions || [];
      const rectorsPermission = getUserPermission(req, 'rectors', ['login']);
      const hcthPermission = getUserPermission(req, 'hcth', ['login']);
      const staffType = rectorsPermission.login
        ? 1
        : hcthPermission.login
        ? 2
        : 0;
      ...
      const data = {
        ...
        donViCanBo:
          donViCanBo.toString() ||
          (userPermissions.includes('donViCongVanDi:manage')
            ? req.session.user?.staff?.maDonVi
            : '') ||
          '',
        staffType,
      ...
      try {
        ...
        app.model.hcthCongVanDi.searchSelector(
          pageNumber,
          pageSize,
          filterParam,
          searchTerm,
  ...
  const updateCongVanDi = (id, changes) =>
    new Promise((resolve, reject) => {
  ...
  const createSoCongVan = (ma, donViGui) =>
    new Promise((resolve, reject) => {
      const currentYear = new Date().getFullYear();
      const firstDayOfYear = new Date(currentYear, 0, 1);
      const nam = Date.parse(firstDayOfYear);
  ...
  app.put(
    '/api/hcth/cong-van-cac-phong/status',
    app.permission.check('staff:login'),
    async (req, res) => {
      try {
        ...
        if (congVan.trangThai == trangThai || !trangThai) {
          ...
          const canBoGui = await app.model.hcthHistory.getStaff(
            { key: id, hanhDong: 'CREATE' },
            'shcc',
            ''
          );
  ...
  app.get(
    '/api/hcth/cong-van-cac-phong/phan-hoi/:id',
    app.permission.check('staff:login'),
    async (req, res) => {
      try {
        const id = parseInt(req.params.id);
        const phanHoi = await app.model.hcthPhanHoi.getAllPhanHoiFrom(
          id,
          CONG_VAN_DI_TYPE
        );
    ...
    new Promise((resolve) => {
      try {
        if (before == after) resolve();
          ...
          createHcthStaffNotification(item, after)
            .then(() => resolve())
            .catch((error) => {
              throw error;
            });
        } else if (after == trangThaiCongVanDi.CHO_DUYET.id) {
          createSchoolAdministratorNotification(item, after)
            .then(() => resolve())
            .catch((error) => {
              throw error;
            });
        } else if (after == trangThaiCongVanDi.DA_GUI.id) {
          createStaffNotification(item, after)
            .then(() => resolve())
            .catch((error) => {
              throw error;
            });
        } else if (after == trangThaiCongVanDi.TRA_LAI.id) {
          createAuthorNotification(item.id, shcc, after)
            .then(() => resolve())
            .catch((error) => {
              throw error;
            });
