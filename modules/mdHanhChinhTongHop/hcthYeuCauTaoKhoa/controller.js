module.exports = app => {
    ...
    const genKey = async (id, shcc, passphrase) => {
        // const fs = require('fs');
        const setting = await app.model.hcthSetting.getValue('passphrase');
        ...
        const caKey = forge.pki.decryptRsaPrivateKey(privateKey, setting.passphrase);
    ...
    app.post('/api/hcth/khoa/user/download', app.permission.orCheck('rectors:login', 'manager:write'), async (req, res) => {
        try {
            ...
            const p12b64 = await genKey(khoa.id, shcc, passphrase);
            res.send({});
        } catch (error) {
            console.error(error);
            res.send({ error });
