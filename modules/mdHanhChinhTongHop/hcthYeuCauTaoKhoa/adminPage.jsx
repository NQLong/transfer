...
export class YeuCauTaoKhoa extends AdminPage {
    ...
    onAprroveRequest = (request) => {
        T.confirm('Duyệt yêu cầu này', `Bạn có chắc muốn duyệt yêu cầu tạo khóa của ${`${request.ho} ${request.ten}`.trim().normalizedName()}?`, true,
            isConfirm => isConfirm && this.props.updateStatus(request.id, trangThaiRequest.DA_DUYET.id));
    }

    onRefuseRequest = (request) => {
        T.confirm('Từ chối yêu cầu này', `Bạn có chắc muốn từ chối yêu cầu tạo khóa của ${`${request.ho} ${request.ten}`.trim().normalizedName()}?`, true,
    ...
    renderTable = (permissions) => renderTable({
        ...
        renderRow: (item, index) => {
            ...
            return <tr key={index}>
                ...
                <TableCell type='buttons' style={{ whiteSpace: 'nowrap', textAlign: 'left' }} content={''} >
