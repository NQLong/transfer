...
module.exports = app => {
    app.model.dmNguonKinhPhiTrongNuoc = {
        ...
        get: (condition, selectedColumns, orderBy, done) => {
            ...
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT * FROM DM_NGUON_KINH_PHI_TRONG_NUOC' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ') WHERE ROWNUM=1';
        ...
        getAll: (condition, selectedColumns, orderBy, done) => {
            ...
            condition = app.dbConnection.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM DM_NGUON_KINH_PHI_TRONG_NUOC' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '');
