...
export function getDtNganhDaoTaoPage(pageNumber, pageSize, pageCondition) {
    ...
    return dispatch => {
        const url = `/api/dao-tao/nganh-dao-tao/page/${page.pageNumber}/${page.pageSize}`;
...
export function createDtNganhDaoTao(item, done) {
    return dispatch => {
        const url = '/api/dao-tao/nganh-dao-tao';
...
export const SelectAdapter_DtNganhDaoTaoFilter = (donVi) => {
    return {
        ...
        processResults: response => ({ results: response && response.items ? response.items.map(item => ({ id: item.maNganh, text: item.tenNganh, khoa: item.khoa })) : [] }),
        fetchOne: (maNganh, done) => (getDtNganhDaoTao(maNganh, item => done && done({ id: item.maNganh, text: item.tenNganh })))(),
    };
};