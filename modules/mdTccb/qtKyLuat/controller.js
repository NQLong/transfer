module.exports = app => {
    ...
    app.get('/api/tccb/qua-trinh/ky-luat/group/page/:pageNumber/:pageSize', app.permission.check('qtKyLuat:read'), (req, res) => {
        const pageNumber = parseInt(req.params.pageNumber),
            ...
            searchTerm = typeof req.query.condition === 'string' ? req.query.condition : '';
        const { fromYear, toYear, list_shcc, list_dv } = (req.query.filter && req.query.filter != '%%%%%%%%%%') ? req.query.filter : { fromYear: null, toYear: null, list_shcc: null, list_dv: null };
        app.model.qtKyLuat.groupPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
                const { totalitem: totalItem, pagesize: pageSize, pagetotal: pageTotal, pagenumber: pageNumber, rows: list } = page;
    ...
    app.get('/api/qua-trinh/ky-luat/download-excel/:list_shcc/:list_dv/:fromYear/:toYear', app.permission.check('qtKyLuat:read'), (req, res) => {
        let { list_shcc, list_dv, fromYear, toYear } = req.params ? req.params : { list_shcc: null, list_dv: null, toYear: null };
        if (list_shcc == 'null') list_shcc = null;
        if (list_dv == 'null') list_dv = null;
        if (fromYear == 'null') fromYear = null;
        if (toYear == 'null') toYear = null;
        app.model.qtKyLuat.download(list_shcc, list_dv, fromYear, toYear, (err, result) => {
            if (err || !result) {
                res.send({ err });
            } else {
                ...
                new Promise(resolve => {
                    let cells = [
                        ...
                        { cell: 'D1', value: 'Số quyết định', bold: true, border: '1234' },
                        { cell: 'E1', value: 'Ngày ra quyết định', bold: true, border: '1234' },
                        { cell: 'F1', value: 'Hình thức kỷ luật', bold: true, border: '1234' },
                        { cell: 'G1', value: 'Cấp quyết định', bold: true, border: '1234' },
                    ...
                    result.rows.forEach((item, index) => {
                        ...
                        cells.push({ cell: 'D' + (index + 2), border: '1234', value: item.soQuyetDinh });
                        cells.push({ cell: 'E' + (index + 2), border: '1234', value: item.ngayRaQuyetDinh ? app.date.dateTimeFormat(new Date(item.ngayRaQuyetDinh), 'dd/mm/yyyy') : ''  });
                        cells.push({ cell: 'F' + (index + 2), border: '1234', value: item.tenKyLuat });
                        cells.push({ cell: 'G' + (index + 2), border: '1234', value: item.capQuyetDinh });
