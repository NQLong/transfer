module.exports = app => {
    app.post('/api/qua-trinh/ky-luat', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyLuat.create(req.body.data, (error, item) => res.send({ error, item })));

    app.put('/api/qua-trinh/ky-luat', app.permission.check('staff:write'), (req, res) =>
        app.model.qtKyLuat.update({ id: req.body.id }, req.body.changes, (error, item) => res.send({ error, item })));

    app.delete('/api/qua-trinh/ky-luat', app.permission.check('staff:write'), (req, res) =>
