    ...
    app.model.qtKyLuat = {
        create: (data, done) => {
            ...
            if (statement.length == 0) {
                ...
                const sql = 'INSERT INTO QT_KY_LUAT (' + statement.substring(2) + ') VALUES (' + values.substring(2) + ')';
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.qtKyLuat.get({ rowId: resultSet.lastRowid }, done);
                    } else {
        ...
        get: (condition, selectedColumns, orderBy, done) => {
            ...
            const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT * FROM QT_KY_LUAT' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ') WHERE ROWNUM=1';
        ...
        getAll: (condition, selectedColumns, orderBy, done) => {
            ...
            condition = app.database.oracle.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM QT_KY_LUAT' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '');
        ...
        getPage: (pageNumber, pageSize, condition, selectedColumns, orderBy, done) => {
            ...
            const sql_count = 'SELECT COUNT(*) FROM QT_KY_LUAT' + (condition.statement ? ' WHERE ' + condition.statement : '');
            app.database.oracle.connection.main.execute(sql_count, parameter, (err, res) => {
                ...
                const sql = 'SELECT ' + app.database.oracle.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT QT_KY_LUAT.*, ROW_NUMBER() OVER (ORDER BY ' + (orderBy ? orderBy : keys) + ') R FROM QT_KY_LUAT' + (condition.statement ? ' WHERE ' + condition.statement : '') + ') WHERE R BETWEEN ' + (leftIndex + 1) + ' and ' + (leftIndex + pageSize);
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
        ...
        update: (condition, changes, done) => {
            ...
            if (changes.statement) {
                ...
                const sql = 'UPDATE QT_KY_LUAT SET ' + changes.statement + (condition.statement ? ' WHERE ' + condition.statement : '');
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.qtKyLuat.get({ rowId: resultSet.lastRowid }, done);
                    } else {
        ...
        delete: (condition, done) => {
            ...
            condition = app.database.oracle.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'DELETE FROM QT_KY_LUAT' + (condition.statement ? ' WHERE ' + condition.statement : '');
        ...
        searchPage: (pagenumber, pagesize, listShcc, listDv, fromyear, toyear, searchterm, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=qt_ky_luat_search_page(:pagenumber, :pagesize, :listShcc, :listDv, :fromyear, :toyear, :searchterm, :totalitem, :pagetotal); END;',
        ...
        groupPage: (pagenumber, pagesize, listShcc, listDv, fromyear, toyear, searchterm, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=qt_ky_luat_group_page(:pagenumber, :pagesize, :listShcc, :listDv, :fromyear, :toyear, :searchterm, :totalitem, :pagetotal); END;',
                { ret: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR }, pagenumber: { val: pagenumber, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, pagesize: { val: pagesize, dir: app.database.oracle.BIND_INOUT, type: app.database.oracle.NUMBER }, listShcc, listDv, fromyear, toyear, searchterm, totalitem: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER }, pagetotal: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.NUMBER } }, (error, result) => app.database.oracle.fetchRowsFromCursor(error, result, done));
        },

        download: (listShcc, listDv, fromyear, toyear, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=qt_ky_luat_download_excel(:listShcc, :listDv, :fromyear, :toyear); END;',
                { ret: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR }, listShcc, listDv, fromyear, toyear }, (error, result) => app.database.oracle.fetchRowsFromCursor(error, result, done));
        },
    };
};