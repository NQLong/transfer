// Table name: QT_DAO_TAO { shcc, tenTruong, chuyenNganh, batDau, ketThuc, hinhThuc, loaiBangCap, id, batDauType, ketThucType, thoiGian }
const keys = ['ID'];
const obj2Db = { 'shcc': 'SHCC', 'tenTruong': 'TEN_TRUONG', 'chuyenNganh': 'CHUYEN_NGANH', 'batDau': 'BAT_DAU', 'ketThuc': 'KET_THUC', 'hinhThuc': 'HINH_THUC', 'loaiBangCap': 'LOAI_BANG_CAP', 'id': 'ID', 'batDauType': 'BAT_DAU_TYPE', 'ketThucType': 'KET_THUC_TYPE', 'thoiGian': 'THOI_GIAN' };
...
module.exports = app => {
    app.model.qtDaoTao = {
        ...
        get: (condition, selectedColumns, orderBy, done) => {
            ...
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT * FROM QT_DAO_TAO' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ') WHERE ROWNUM=1';
        ...
        getAll: (condition, selectedColumns, orderBy, done) => {
            ...
            condition = app.dbConnection.buildCondition(obj2Db, condition, ' AND ');
            const parameter = condition.parameter ? condition.parameter : {};
            const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM QT_DAO_TAO' + (condition.statement ? ' WHERE ' + condition.statement : '') + (orderBy ? ' ORDER BY ' + orderBy : '');
        ...
        getPage: (pageNumber, pageSize, condition, selectedColumns, orderBy, done) => {
            ...
            app.dbConnection.execute(sql_count, parameter, (err, res) => {
                ...
                const sql = 'SELECT ' + app.dbConnection.parseSelectedColumns(obj2Db, selectedColumns) + ' FROM (SELECT QT_DAO_TAO.*, ROW_NUMBER() OVER (ORDER BY ' + (orderBy ? orderBy : keys) + ') R FROM QT_DAO_TAO' + (condition.statement ? ' WHERE ' + condition.statement : '') + ') WHERE R BETWEEN ' + (leftIndex + 1) + ' and ' + (leftIndex + pageSize);
                app.dbConnection.execute(sql, parameter, (error, resultSet) => {
