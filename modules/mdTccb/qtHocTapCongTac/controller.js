module.exports = app => {
    ...
    app.get('/user/tccb/qua-trinh/hoc-tap-cong-tac/group_htct/:loaiDoiTuong/:ma', app.permission.check('qtHocTapCongTac:read'), app.templates.admin);
    ...
    app.get('/api/tccb/qua-trinh/hoc-tap-cong-tac/group/page/:pageNumber/:pageSize', app.permission.check('qtHocTapCongTac:read'), (req, res) => {
        ...
        app.model.qtHocTapCongTac.groupPage(pageNumber, pageSize, loaiDoiTuong, fromYear, toYear, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
                const { totalitem: totalItem, pagesize: pageSize, pagetotal: pageTotal, pagenumber: pageNumber, rows: list } = page;
