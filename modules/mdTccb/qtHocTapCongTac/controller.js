module.exports = app => {
    ...
    const menuStaff = {
        ...
        menus: {
            1012: { title: 'Học tập, công tác', link: '/user/hoc-tap-cong-tac', icon: 'fa-calendar', backgroundColor: '#30a17f', groupIndex: 1 },
        },
    };
    ...
    app.put('/api/user/qua-trinh/hoc-tap-cong-tac', app.permission.check('staff:login'), (req, res) => {
    ...
    app.delete('/api/user/qua-trinh/hoc-tap-cong-tac', app.permission.check('staff:login'), (req, res) => {
        if (req.session.user) {
            app.model.qtHocTapCongTac.get({ id: req.body.id }, (error, item) => {
                if (error || item == null) {
                    res.send({ error: 'Not found!' });
    ...
    app.get('/api/user/qua-trinh/hoc-tap-cong-tac/page/:pageNumber/:pageSize', app.permission.check('staff:login'), (req, res) => {
        ...
        app.model.qtHocTapCongTac.searchPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, timeType, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
    ...
    app.get('/api/tccb/qua-trinh/hoc-tap-cong-tac/group/page/:pageNumber/:pageSize', app.permission.check('qtHocTapCongTac:read'), (req, res) => {
        const pageNumber = parseInt(req.params.pageNumber),
            ...
            searchTerm = typeof req.query.condition === 'string' ? req.query.condition : '';
        const { fromYear, toYear, list_shcc, list_dv, timeType, tinhTrang } = (req.query.filter && req.query.filter != '%%%%%%%%%%') ? req.query.filter : { fromYear: null, toYear: null, list_shcc: null, list_dv: null, timeType: 0, tinhTrang: null };
        app.model.qtHocTapCongTac.groupPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, timeType, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
                const { totalitem: totalItem, pagesize: pageSize, pagetotal: pageTotal, pagenumber: pageNumber, rows: list } = page;
