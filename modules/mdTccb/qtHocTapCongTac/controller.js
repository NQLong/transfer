module.exports = app => {
    ...
    app.get('/api/user/qua-trinh/hoc-tap-cong-tac/page/:pageNumber/:pageSize', app.permission.check('staff:login'), (req, res) => {
        ...
        app.model.qtHocTapCongTac.searchPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
    ...
    app.get('/api/tccb/qua-trinh/hoc-tap-cong-tac/group/page/:pageNumber/:pageSize', app.permission.check('qtHocTapCongTac:read'), (req, res) => {
        const pageNumber = parseInt(req.params.pageNumber),
            ...
            searchTerm = typeof req.query.condition === 'string' ? req.query.condition : '';
        const { fromYear, toYear, list_shcc, list_dv, tinhTrang } = (req.query.filter && req.query.filter != '%%%%%%%%%%') ? req.query.filter : { fromYear: null, toYear: null, list_shcc: null, list_dv: null, tinhTrang: null };
        app.model.qtHocTapCongTac.groupPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
                const { totalitem: totalItem, pagesize: pageSize, pagetotal: pageTotal, pagenumber: pageNumber, rows: list } = page;
