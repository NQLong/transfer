module.exports = app => {
    // APIs -----------------------------------------------------------------------------------------------------------------------------------------
    // app.get('/api/tccb/danh-gia/diem-thuong/page/:pageNumber/:pageSize', app.permission.check('tccbDanhGiaNam:manage'), (req, res) => {
    ...
    app.get('/api/tccb/danh-gia/diem-thuong/all', app.permission.check('tccbDanhGiaNam:manage'), (req, res) => {
        const condition = req.query.condition || {};
        app.model.tccbDiemThuong.getAll(condition, '*', 'id', (error, items) => res.send({ error, items }));
    });

    app.get('/api/tccb/danh-gia/diem-thuong/item/:id', app.permission.check('tccbDanhGiaNam:manage'), (req, res) => {
        app.model.tccbDiemThuong.get({ id: req.params.id }, (error, item) => res.send({ error, item }));
    });

    app.post('/api/tccb/danh-gia/diem-thuong', app.permission.check('tccbDanhGiaNam:write'), (req, res) => {
    ...
    app.put('/api/tccb/danh-gia/diem-thuong', app.permission.check('tccbDanhGiaNam:write'), (req, res) => {
