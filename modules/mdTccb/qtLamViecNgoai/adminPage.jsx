...
class EditModal extends AdminModal {
    ...
    onShow = (item, multiple = true) => {
        ...
        let { id, shcc, noiDung, batDau, batDauType, ketThuc, ketThucType, noiLamViec } = item ? item : {
            id: '', shcc: '', noiDung: '', batDau: null, batDauType: '', ketThuc: null, ketThucType: '', noiLamViec: ''
        };
        ...
        this.setState({
            ...
            this.noiLamViec.value(noiLamViec ? noiLamViec : '');
...
class QtLamViecNgoai extends AdminPage {
    ...
    changeAdvancedSearch = (isInitial = false) => {
        let { pageNumber, pageSize } = this.props && this.props.qtLamViecNgoai && this.props.qtLamViecNgoai.page ? this.props.qtLamViecNgoai.page : { pageNumber: 1, pageSize: 50 };
        ...
        const pageFilter = isInitial ? null : { listDv, fromYear, toYear, listShcc, tinhTrang };
        this.setState({ filter: pageFilter }, () => {
            this.getPage(pageNumber, pageSize, '', (page) => {
                if (isInitial) {
                    ...
                    if (!$.isEmptyObject(filter) && filter && (filter.fromYear || filter.toYear || filter.listShcc || filter.listDv || filter.tinhTrang)) this.showAdvanceSearch();
                }
            });
        });
    }
    ...
    getPage = (pageN, pageS, pageC, done) => {
        if (this.checked) this.props.getQtLamViecNgoaiGroupPage(pageN, pageS, pageC, this.state.filter, done);
        else this.props.getQtLamViecNgoaiPage(pageN, pageS, pageC, this.state.filter, done);
    }
    ...
    list = (text, i, j) => {
        if (!text) return [];
        let items = text.split('??').map(str => <div key={i--}>{j - i}. {str}</div>);
        return items;
    }

    delete = (e, item) => {
        T.confirm('Xóa quá trình làm việc ngoài', 'Bạn có chắc bạn muốn xóa quá trình làm việc ngoài này?', 'warning', true, isConfirm => {
            isConfirm && this.props.deleteQtLamViecNgoaiStaff(item.id, error => {
                if (error) T.notify(error.message ? error.message : 'Xoá quá trình làm việc ngoài bị lỗi!', 'danger');
                else T.alert('Xoá quá trình làm việc ngoài thành công!', 'success', false, 800);
            });
        });
        e.preventDefault();
    }
    ...
    render() {
        const currentPermissions = this.props.system && this.props.system.user && this.props.system.user.permissions ? this.props.system.user.permissions : [],
            permission = this.getUserPermission('qtLamViecNgoai', ['read', 'write', 'delete']);
        let { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.checked ? (
            this.props.qtLamViecNgoai && this.props.qtLamViecNgoai.pageGr ?
                this.props.qtLamViecNgoai.pageGr : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, list })
            : (this.props.qtLamViecNgoai && this.props.qtLamViecNgoai.page ? this.props.qtLamViecNgoai.page : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list: [] });
            ...
            table = renderTable({
                ...
                renderRow: (item, index) => (
                    <tr key={index}>
                        ...
                        {this.checked && <TableCell type='text' content={this.list(item.danhSachNoiDung, item.soQuaTrinh, item.soQuaTrinh)} />}
                        {
                        ...
                        {
                            this.checked && <TableCell type='buttons' style={{ textAlign: 'center' }} content={item} permission={permission}>
                                <Link className='btn btn-success' to={`/user/tccb/qua-trinh/lam-viec-ngoai/group/${item.shcc}`} >
        ...
        return this.renderPage({
            icon: 'fa fa fa-fax',
            title: ' Quá trình làm việc ngoài',
            breadcrumb: [
