...
import {
    getQtLamViecNgoaiUserPage, deleteQtLamViecNgoaiUserPage, createQtLamViecNgoaiUserPage,
    updateQtLamViecNgoaiUserPage
}
    from './redux';
...
class EditModal extends AdminModal {
    ...
    onShow = (item) => {
        let { id, noiDung, batDau, batDauType, ketThuc, ketThucType, noiLamViec } = item && item.item ? item.item : {
            id: '', noiDung: '', tenCoSo: '', batDau: null, batDauType: '', ketThuc: null, ketThucType: '', noiLamViec: ''
        };
        this.setState({
...
class QtLamViecNgoaiUserPage extends AdminPage {
    state = { filter: {} };
    componentDidMount() {
        T.ready('/user', () => {
            const { shcc } = this.props.system && this.props.system.user ? this.props.system.user : { shcc: '' };
            this.setState({ filter: { listShcc: shcc, listDv: '', fromYear: null, toYear: null, tinhTrang: null } });
            this.getPage();
        });
    }

    getPage = (pageN, pageS, pageC, done) => {
        this.props.getQtLamViecNgoaiUserPage(pageN, pageS, pageC, this.state.filter, done);
    }
    ...
    delete = (e, item) => {
        T.confirm('Xóa thông tin làm việc ngoài', 'Bạn có chắc bạn muốn xóa thông tin làm việc ngoài này?', 'warning', true, isConfirm => {
            isConfirm && this.props.deleteQtLamViecNgoaiUserPage(item.id, error => {
    ...
    render() {
        ...
        let { pageNumber, pageSize, pageTotal, totalItem, pageCondition, list } = this.props.qtLamViecNgoai && this.props.qtLamViecNgoai.userPage ? this.props.qtLamViecNgoai.userPage : { pageNumber: 1, pageSize: 50, pageTotal: 1, totalItem: 0, pageCondition: {}, list: [] };
        ...
        return this.renderPage({
            ...
            content: <>
                ...
                <EditModal ref={e => this.modal = e} shcc={this.shcc} readOnly={!permission.write}
                    create={this.props.createQtLamViecNgoaiUserPage} update={this.props.updateQtLamViecNgoaiUserPage}
                />
            </>,
            backRoute: '/user',
...
const mapActionsToProps = {
    getQtLamViecNgoaiUserPage, deleteQtLamViecNgoaiUserPage,
    updateQtLamViecNgoaiUserPage, createQtLamViecNgoaiUserPage,
};
export default connect(mapStateToProps, mapActionsToProps)(QtLamViecNgoaiUserPage);