module.exports = app => {
    ...
    function calcSoNgayPhepConLai(shcc, ngayBatDauCongTac, current, done) {
        new Promise(resolve => {
                ...
                const solve = (idx = 0) => {
                    ...
                    if (year == currentYear) {
                        app.model.dmNghiPhep.get({ ma: items[idx].lyDo }, (error, itemNghiPhep ) => {
                            let value = Math.max(app.numberNgayNghi(new Date(items[idx].batDau), new Date(items[idx].ketThuc)) - itemNghiPhep.soNgayPhep, 0);
                            result -= value;
                            solve(idx + 1);
                        });
                    }
            ...
            if (error || !result) {
                ...
                new Promise(resolve => {
                    ...
                    const solve = (index = 0) => {
                        ...
                        calcSoNgayPhepConLai(item.shcc, item.ngayBatDauCongTac, item.ngayNghiPhep, soNgayPhepConLai => {
                            ...
                            cells.push({ cell: 'M' + (index + 2), border: '1234', value: app.numberNgayNghi(new Date(item.batDau), new Date(item.ketThuc)) });
                            cells.push({ cell: 'N' + (index + 2), border: '1234', value: Math.max(app.numberNgayNghi(new Date(item.batDau), new Date(item.ketThuc)) - item.ngayNghiPhep, 0) });
