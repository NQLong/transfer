module.exports = app => {
    const menu = {
        ...
        menus: {
            3041: { title: 'Quá trình nghỉ phép', link: '/user/tccb/qua-trinh/nghi-phep', icon: 'fa fa-calendar-o', color: '#000000', backgroundColor: '#850de0', groupIndex: 4 },
        },
    };
    const menuStaff = {
        parentMenu: app.parentMenu.user,
        menus: {
            1026: { title: 'Nghỉ phép', link: '/user/nghi-phep', icon: 'fa fa-calendar-o', backgroundColor: '#850de0', groupIndex: 1 },
        },
    };
    ...
    app.get('/api/user/qua-trinh/nghi-phep/page/:pageNumber/:pageSize', app.permission.check('staff:login'), (req, res) => {
        ...
        app.model.qtNghiPhep.searchPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
    ...
    app.get('/api/tccb/qua-trinh/nghi-phep/group/page/:pageNumber/:pageSize', app.permission.check('qtNghiPhep:read'), (req, res) => {
        const pageNumber = parseInt(req.params.pageNumber),
            ...
            searchTerm = typeof req.query.condition === 'string' ? req.query.condition : '';
        const { fromYear, toYear, list_shcc, list_dv, tinhTrang } = (req.query.filter && req.query.filter != '%%%%%%%%%%') ? req.query.filter : { fromYear: null, toYear: null, list_shcc: null, list_dv: null, tinhTrang: null };
        app.model.qtNghiPhep.groupPage(pageNumber, pageSize, list_shcc, list_dv, fromYear, toYear, tinhTrang, searchTerm, (error, page) => {
            if (error || page == null) {
                res.send({ error });
            } else {
                const { totalitem: totalItem, pagesize: pageSize, pagetotal: pageTotal, pagenumber: pageNumber, rows: list } = page;
