...
module.exports = app => {
    app.model.tcHocPhiTransaction = {
            ...
            if (statement.length == 0) {
                ...
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.tcHocPhiTransaction.get({ rowId: resultSet.lastRowid }, done);
                    } else {
        ...
        addBill: (namhoc, hocky, ebank, transid, transdate, customerid, billid, serviceid, eamount, echecksum, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=tc_hoc_phi_transaction_add_bill(:namhoc, :hocky, :ebank, :transid, :transdate, :customerid, :billid, :serviceid, :eamount, :echecksum); END;',
        ...
        searchPage: (pagenumber, pagesize, searchterm, filter, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=tc_hoc_phi_transaction_search_page(:pagenumber, :pagesize, :searchterm, :filter, :totalitem, :pagetotal); END;',
        ...
        downloadPsc: (filter, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=tc_hoc_phi_transaction_download_psc(:filter); END;',
        ...
        listBank: (searchterm, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=tc_hoc_phi_transaction_list_bank(:searchterm); END;',
