...
module.exports = app => {
    app.model.tcHocPhi = {
            ...
            if (statement.length == 0) {
                ...
                app.database.oracle.connection.main.execute(sql, parameter, (error, resultSet) => {
                    if (error == null && resultSet && resultSet.lastRowid) {
                        app.model.tcHocPhi.get({ rowId: resultSet.lastRowid }, done);
                    } else {
        ...
        getAllFeeOfStudent: (imssv, done) => {
            app.database.oracle.connection.main.execute('BEGIN :ret:=tc_hoc_phi_get_all_mssv(:imssv, :hocphidetailall); END;',
                { ret: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR }, imssv, hocphidetailall: { dir: app.database.oracle.BIND_OUT, type: app.database.oracle.CURSOR } }, (error, result) => app.database.oracle.fetchRowsFromCursor(error, result, done));
        },
